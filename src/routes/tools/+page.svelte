...
<svelte:head>
    ...
    <meta name="description" content="Developer utility tools for everyday tasks" />
</svelte:head>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="text-center mb-16 page-enter">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6"
                ...
                <i class="fa-solid fa-wrench text-2xl" style="color: var(--accent);"></i>
            </div>
        ...
        <div class="max-w-xl mx-auto mb-12">
            <SearchInput bind:value={searchQuery} placeholder="Search tools..." />
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-2 mb-12">
            <button
                onclick={() => selectedTag = ''}
                class="tag cursor-pointer {selectedTag === '' ? 'tag-accent' : ''}"
            >
                All
            </button>
            {#each allTags as tag}
                <button
                    ...
                    class="tag cursor-pointer {selectedTag === tag ? 'tag-accent' : ''}"
                >
                    {tag}
                </button>
            {/each}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {#each filteredTools as tool, i}
                <a href="/tools/{tool.slug}"
                    ...
                    <div class="w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-4 transition-all duration-300 group-hover:scale-110"
                        style="background: var(--bg-tertiary);">
                        <i class="{tool.icon} text-2xl" style="color: var(--accent);"></i>
                    </div>

                    <h2 class="font-display font-bold text-lg mb-2 group-hover:text-accent transition-colors"
        ...
        {#if filteredTools.length === 0}
            <div class="text-center py-16">
                ...
                <p class="text-lg mb-2" style="color: var(--text-primary);">No tools found</p>
